...
def multi_circular_shifts(word, K): #This code stores the original word in the second column of the array
        """Generate all circular shifts of length K for a given word."""
    shifts = []
    n = len(word)
    for i in range(n):
        if(len(word[i:])>3):
            new_elem = [word[i:], word]
            shifts.append(new_elem) #Desde i en adelande
        if(len(word[:i])>3):
            new_elem = [word[:i],word]
            shifts.append() #Desde i atras
...
for i in range(1, len(inputArgs)):
    variations.extend(multi_circular_shifts(sys.argv[i], 12)) #I should store the  initial word
