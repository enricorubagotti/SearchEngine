...
for row in  sorted_data:
    ...
    if (int(row[0]) != file_before):
        ...
        word_before=int(row[2])
        point_before=int(row[5])
    elif ((int(row[0]) == file_before) and (int(row[1]) ==  page_before) and ((word_before-int(row[2]))<5)):
          row[5]=float(point_before)+float(abs(row[3]/(word_before-row[2])))
          row[6]+=1

        
#    print(row)
sorted_data_by_point = sorted(tabular_result, key=lambda x: x[6], reverse=True )
