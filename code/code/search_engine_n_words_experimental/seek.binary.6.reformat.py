...
freq_file_page_word_N={} #It maps the file-page name and the word number to the frequency
        ...
        for i in range(1, len(coordinates_array)-1, 2):
            try:
                if freq_file_page_word_N.get(str(coordinates_array[i]+"_"+coordinates_array[i+1])) is not None:
                    freq_file_page_word_N[str(coordinates_array[i]+"_"+coordinates_array[i+1])]+=len(coordinates_array[0])#This is the word length
                                                                                                                          #on the index 
                else:
                    freq_file_page_word_N[str(coordinates_array[i]+"_"+coordinates_array[i+1])]=len(coordinates_array[0])
