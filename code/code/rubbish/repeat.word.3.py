...
for i in range(1, len(sys.argv)):
    subprocess.call(['python3', 'seek.binary.position.2.py', str(sys.argv[i])], stdout=f)
f.close()
...
for line in f:
    ...
    elements = line.split("\t")
    if len(elements) == 2:
        tabular_result.append([float(elements[0]), float(elements[1]),float(0)])
    line_n_out += 1
...
#    [tabular_result[0][lineN], tabular_result[1][lineN]]=line.split("\t")
#    lineN+=1
#f.close()

# Sort the data based on the first column (index 0)
sorted_data = sorted(tabular_result, key=lambda x: x[0], reverse=True)
